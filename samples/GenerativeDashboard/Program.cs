using A2A;
using GenerativeDashboard;
using GenerativeDashboard.Components;
using Microsoft.Agents.AI.Hosting;
using Microsoft.AspNetCore.Components;
using Microsoft.Extensions.AI;
using Strathweb.A2UI;
using Strathweb.A2UI.A2A;
using Strathweb.A2UI.AgentFramework;
using Strathweb.A2UI.Catalogs;
using Strathweb.A2UI.Prompting;
using Strathweb.A2UI.Samples;

var builder = WebApplication.CreateBuilder(args);

var demoLogs = new DemoLogProvider();
builder.Logging.AddProvider(demoLogs);
builder.Services.AddSingleton(demoLogs);

var catalog = A2UICatalogs.Basic(A2UIVersion.V0_9_1);
var chatClient = SampleModels.TryCreateOpenAI(builder.Configuration) ?? new DashboardScript();

// Prompt-first: the system prompt teaches the model to write A2UI JSON itself, and the agent turns
// the blocks it writes into surfaces, validating each against the catalog and asking for a fix when
// one is wrong.
var instructions = A2UISystemPrompt.Generate(catalog, new A2UIPromptOptions
{
    RoleDescription = Dashboard.Role,
    UiDescription = Dashboard.UiDescription,
    IncludeSchema = true,
});

var agent = chatClient
    .AsAIAgent(instructions: instructions, name: "sales-analyst", description: "Answers sales questions with dashboards it draws itself.")
    .WithA2UI(options =>
    {
        options.PromptFirst = new A2UIPromptFirstOptions { Catalog = catalog, MaxModelRepairs = 1 };
        options.LoggerFactory = LoggerFactory.Create(logging => logging.AddProvider(demoLogs));
    });

builder.Services.AddKeyedSingleton<AgentSessionStore>(agent.Name, new InMemoryAgentSessionStore());
builder.Services.AddA2AServer(agent);

builder.Services.AddRazorComponents().AddInteractiveServerComponents();
builder.Services.AddHttpClient();
builder.Services.AddScoped(services => new A2UIChatSession(() => new A2AClient(
    new Uri(new Uri(services.GetRequiredService<NavigationManager>().BaseUri), "a2a"),
    services.GetRequiredService<IHttpClientFactory>().CreateClient())));
builder.Services.AddSingleton(new SampleInfo(SampleModels.Describe(chatClient)));

var app = builder.Build();

app.UseStaticFiles();
app.UseAntiforgery();

app.MapA2AJsonRpc(agent, "/a2a");
app.MapGet("/.well-known/agent-card.json", () => new AgentCard
{
    Name = agent.Name!,
    Description = agent.Description ?? string.Empty,
    Version = "0.1.0",
    DefaultInputModes = ["text/plain"],
    DefaultOutputModes = ["text/plain", A2UIMediaTypes.A2UIJson],
    Capabilities = new AgentCapabilities { Streaming = true },
}.AddA2UIExtension(A2UIVersion.V0_9_1, supportedCatalogIds: [catalog.CatalogId]));

// The prompt the model gets, for the curious.
app.MapGet("/prompt", () => Results.Text(instructions, "text/plain"));

app.MapRazorComponents<App>().AddInteractiveServerRenderMode();

app.Run();
